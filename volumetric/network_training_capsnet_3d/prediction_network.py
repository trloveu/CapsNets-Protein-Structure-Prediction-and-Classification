...
def manipulate_latent(model, data, args):
    ...
    mss = np.mean(voxel_recons, axis = (1, 0, 2))
    vss = np.var(voxel_recons, axis = (1, 0, 2))
    ...
    sh = voxel_scores.shape
    print("Normalizing")
    for i in range(sh[0]):
        for j in range(sh[1]):
            for k in range(sh[2]):
                for l in range(sh[3]):
                    voxel_recons[i][j][k][l] = normalize(voxel_recons[i][j][k][l], mss[l], vss[l])

    voxel_recons = voxel_recons.astype(int)

    predt_x, predt_y, predt_z, predt_w = voxel_recons.nonzero()

    in_x, in_y, in_z, in_w = x[0].nonzero()

    print("Plotting Prediction")
    plotScatter((predt_x, predt_y, predt_z), '3dpred', 'red') 

    print("Plotting Input")
    plotScatter((in_x, in_y, in_z), '3dinput', 'green')
...
def plotScatter(data, name, color):
    ...
    fig = plt.figure()
    ax = Axes3D(fig)
    ax.scatter(data[0], data[1], data[2], color = color, s=[1] * len(data[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dim_type', default = '-3d')
